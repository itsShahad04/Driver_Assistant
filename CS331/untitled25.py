...
display_output = new_output.copy()

max_width = 1200
if display_output.shape[1] > max_width:
    scale = max_width / display_output.shape[1]
    new_w = int(display_output.shape[1] * scale)
    new_h = int(display_output.shape[0] * scale)
    display_output = cv2.resize(display_output, (new_w, new_h))

cv2.imshow("Smart Driver Assistant", display_output)
cv2.waitKey(0)
